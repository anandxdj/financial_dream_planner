...
export class PostgresRunStore implements RunStore {
  ...
  async cancel(id: string) {
    const cancelled = (
      await this.database
        .update(jobRuns)
        .set({ status: "cancelled", cancelRequestedAt: new Date(), completedAt: new Date() })
    ...
    return await this.database.transaction(async (tx) => {
      ...
      const [row] = await tx
        ...
        .values({
          ...
          type,
          payload,
          sequence: sql<number>`(select coalesce(max(sequence), 0) + 1 from run_events where run_id = ${runId})`,
        })
        .returning();
      return row!;
    });
    ...
    if (eventId) {
      const anchor = (
        await this.database
          .select()
          .from(runEvents)
          .where(and(eq(runEvents.id, eventId), eq(runEvents.runId, runId)))
          .limit(1)
      )[0];
      if (!anchor) {
        throw new AppError(400, "INVALID_EVENT_CURSOR", "Event cursor does not belong to this run");
      }
...
function runBelongsToHousehold(run: SelectJobRun, householdId: string): boolean {
  return run.input?.householdId === householdId;
}
...
export class RunService {
  ...
  async getForHousehold(id: string, householdId: string) {
  ...
  async cancel(id: string) {
    const run = await this.store.cancel(id);
    if (!run) throw new AppError(404, "RUN_NOT_FOUND", "Run not found");
    return run;
  }

  async cancelForHousehold(id: string, householdId: string) {
    await this.getForHousehold(id, householdId);
    return this.cancel(id);
  }

  async appendEvent(runId: string, type: RunEventType, payload: Record<string, unknown>) {
  ...
  async eventsAfter(runId: string, eventId?: string) {
    await this.get(runId);
    return this.store.eventsAfter(runId, eventId);
  }

  async eventsAfterForHousehold(runId: string, householdId: string, eventId?: string) {
    await this.getForHousehold(runId, householdId);
    return this.store.eventsAfter(runId, eventId);
  }

  eventsAfterKnownRun(runId: string, eventId?: string) {
    return this.store.eventsAfter(runId, eventId);
  }
}
