...
export async function subscribeRun(
  ...
  const response = await fetch(`${origin}/api/v1/runs/${encodeURIComponent(runId)}/events`, {
    credentials: "include",
  ...
  try {
    while (true) {
      ...
      if (new TextEncoder().encode(buffer).byteLength > maxBufferBytes) {
        throw new Error("Run stream frame exceeds maximum buffer size");
      }
      ...
      for (const frame of frames) {
        ...
        for (const line of frame.split(/\r?\n/)) {
          ...
          if (key === "data") {
            fields.set(key, `${fields.get(key) ? `${fields.get(key)}\n` : ""}${value}`);
          } else {
