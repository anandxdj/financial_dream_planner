...
import { useCurrentPlan, useAccounts } from "@/features/planner/queries";
import { usePlanning } from "@/features/planner/planning-queries";
import { useLoanCalculation, useLoans, type LoanRequestBody } from "@/features/planner/decision-queries";
import {
  action,
  secondary,
  control,
...
export function LoansOverview({
  ...
  // Quick Affordability Checker (Board 08 Panel 07)
  const [affordabilityTab, setAffordabilityTab] = useState<"home" | "car" | "education" | "travel" | "custom">("home");
  const [propertyValue, setPropertyValue] = useState("8000000");
  const [downPaymentPercent, setDownPaymentPercent] = useState("20");
  const [quickTenureYears, setQuickTenureYears] = useState("20");
  const [quickRate, setQuickRate] = useState("8.5");

  function handleTabChange(tab: "home" | "car" | "education" | "travel" | "custom") {
    setAffordabilityTab(tab);
    if (tab === "home") {
      setPropertyValue("8000000");
      setDownPaymentPercent("20");
      setQuickTenureYears("20");
      setQuickRate("8.5");
    } else if (tab === "car") {
      setPropertyValue("1500000");
      setDownPaymentPercent("20");
      setQuickTenureYears("5");
      setQuickRate("9.0");
    } else if (tab === "education") {
      setPropertyValue("2500000");
      setDownPaymentPercent("15");
      setQuickTenureYears("7");
      setQuickRate("10.5");
    } else if (tab === "travel") {
      setPropertyValue("500000");
      setDownPaymentPercent("20");
      setQuickTenureYears("2");
      setQuickRate("12.0");
    }
  }

  function applyQuickAffordability() {
    const totalVal = Number(propertyValue) || 0;
    const downPct = Number(downPaymentPercent) || 0;
    const downVal = (totalVal * downPct) / 100;
    const loanPrinc = String(Math.max(0, totalVal - downVal));
    const tenureMo = String((Number(quickTenureYears) || 1) * 12);
    setPrincipal(loanPrinc);
    setAnnualRate(quickRate);
    setTenureMonths(tenureMo);
    setActiveRequest({
      principal: loanPrinc,
      annualRate: quickRate,
      tenureMonths: Number(tenureMo),
    });
  }
  ...
  return (
    <div className="space-y-10 pb-16">
      ...
      {/* ========================================================================= */}
      {/* 07 QUICK AFFORDABILITY CHECKER (Board 08 Panel 07) */}
      {/* ========================================================================= */}
      <section aria-labelledby="quick-affordability-heading" className="space-y-6">
        <div>
          <span className="text-xs font-bold tracking-wider uppercase text-[#5E55C9]">
            07 Quick Affordability Checker
          </span>
          <h2
            id="quick-affordability-heading"
            className="text-2xl md:text-3xl font-serif text-[#1F2A44]"
          >
            Can you afford it?
          </h2>
          <p className="mt-1 text-sm md:text-base text-[#475467]">
            Get an instant estimate to see what's possible.
          </p>
        </div>

        <div className="rounded-2xl border border-[#E8E1D6] bg-[#FFFCF8] p-6 sm:p-7 shadow-xs">
          {/* Preset Tabs */}
          <div className="flex flex-wrap gap-2 border-b border-[#E8E1D6]/70 pb-4 mb-6">
            {[
              { id: "home", label: "Buy a Home" },
              { id: "car", label: "Buy a Car" },
              { id: "education", label: "Child's Education" },
              { id: "travel", label: "Travel Goal" },
              ...
              <button
                ...
                className={`rounded-full px-4 py-1.5 text-xs font-semibold transition-colors ${
                  affordabilityTab === tab.id
                    ? "bg-[#5E55C9] text-[#FFFCF8]"
                    : "bg-[#FFF9F0] border border-[#E8E1D6] text-[#475467] hover:bg-[#E8E1D6]/40"
                }`}
                onClick={() => handleTabChange(tab.id as "home" | "car" | "education" | "travel" | "custom")}
              >
                {tab.label}
              </button>
            ))}
          ...
          <div className="grid gap-8 lg:grid-cols-12 items-stretch">
            ...
            <div className="lg:col-span-7 space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-1.5">
                  <label className="block text-xs font-semibold text-[#1F2A44]">
                    Property value / Cost (INR)
                  </label>
                  <input
                    type="text"
                    value={propertyValue}
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex gap-2">
                    <input
                      type="text"
                      readOnly
                      value={`₹ ${((Number(propertyValue) * Number(downPaymentPercent)) / 100).toLocaleString("en-IN")}`}
                      className={`${control} bg-[#FFF9F0]/60`}
                    />
                    <select
                      value={downPaymentPercent}
                      onChange={(e) => setDownPaymentPercent(e.target.value)}
                      className="w-24 rounded-[10px] border border-[#E8E1D6] bg-[#FFFCF8] px-2 text-xs font-medium"
                    >
                ...
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    onChange={(e) => setQuickTenureYears(e.target.value)}
                    className={control}
                  >
              ...
              <button
                type="button"
                onClick={applyQuickAffordability}
            ...
            {/* Result Preview Card */}
            <div className="lg:col-span-5 rounded-2xl border border-[#CBD5E1] bg-gradient-to-b from-[#F0FDF4] to-[#FFFCF8] p-6 flex flex-col justify-between shadow-xs">
              <div className="space-y-4">
                <div>
                  <span className="text-xs font-semibold text-[#475467]">
                    Estimated monthly EMI
                  </span>
                  <p className="mt-1 text-3xl sm:text-4xl font-serif font-bold text-[#166534] tabular-nums">
                    {result ? money(result.monthlyEmi) : "₹ 41,822"}
                  </p>
                </div>

                <div className="border-t border-[#E8E1D6]/70 pt-3 space-y-2 text-xs">
                  <div className="flex justify-between">
                    <span className="text-[#475467]">Loan amount</span>
                    <span className="font-semibold text-[#1F2A44] tabular-nums">
                      {result ? money(result.totalPrincipal) : "₹ 64,00,000"}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[#475467]">Total interest</span>
                    <span className="font-semibold text-[#8A531D] tabular-nums">
                      {result ? money(result.totalInterest) : "₹ 36,37,280"}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[#475467]">Total payment</span>
                    <span className="font-semibold text-[#1F2A44] tabular-nums">
                      {result ? money(result.totalPayment) : "₹ 1,00,37,280"}
                    </span>
                  </div>
                </div>
              </div>

              <div className="mt-4 pt-3 border-t border-[#CBD5E1]/60 flex items-center gap-2 text-xs text-[#166534] font-medium">
                <span className="size-5 rounded-full bg-[#DCFCE7] text-[#166534] flex items-center justify-center shrink-0">
                  ✓
                </span>
                <span>Looks good! This fits within your plan.</span>
              </div>
            </div>
          </div>
        </div>
